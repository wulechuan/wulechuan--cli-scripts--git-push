 ...
 * 2022-05-19
 *
 * 本文件为本工具集之一整套测试任务集之入口。
 * 该套测试集可配置，并可反复执行。目前本工具集会连续运行两遍该套测试任务集，两遍的配置参数略有区别。
 *
 * 本文件所测试之内容基本等同于
 *     “1-进行自动安装与配置.js”
 * 中的任务。
 ...
 *
 * @param {string}  配置项集.本套测试任务集之称谓
 *
 * @returns {Promise<any>}
 */
module.exports = function 执行全套测试任务 ({
    应开启调试功能 = false,
    ...
    本套测试任务集之称谓 = '<不知名的测试任务集>',
} = {}) {
    应开启调试功能 && console.debug(`>>>> ${本套测试任务集之称谓}: 执行全套测试任务 <<<<`)
    ...
    return Promise.all([
        ...
        应开启调试功能 && console.debug(`>>>> ${本套测试任务集之称谓}: 执行全套测试任务 then <<<<`)
        ...
        if (出现异常) {
            console.error(`\n${
                ...
                粉笔工具.bgRed.whiteBright(' 在仿真环境中自动配置失败 ')
            } 。\n\n    ${
                '虽然本工具集的“自动配置”未能成功，'
            }\n    ${
