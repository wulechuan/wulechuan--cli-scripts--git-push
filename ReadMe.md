...
>
> 又，虽然本套工具之主体并非 JavaScript 程序，但本工具集仍内建了一些 JavaScript 程序，且它们确有其价值。当其他 npm 包（暂称**甲**）安装了本工具集后，本工具集内建的那些 JavaScript 程序会自动化的改动甲的环境配置，以期令甲更方便的利用本工具集。
...
1.  正式安装本工具集。
    ...
    -  本工具会尽量将一些命令行脚本文件复制到 `<甲>/用于研发阶段的命令行工具/PowerShell/` 文件夹内。如果此文件夹中原先已有同名脚本文件，则这些旧文件将得到保留，不会被覆盖。**但自本工具集之 `v4.0.1` 版始，JavaScript 程序会尽可能修订业已存在的就命令行脚本文件。**
    -  本工具会尽量将一些命令行脚本文件复制到 `<甲>/用于研发阶段的命令行工具/bash/` 文件夹内。如果此文件夹中原先已有同名脚本文件，则这些旧文件将得到保留，不会被覆盖。**但自本工具集之 `v4.0.1` 版始，JavaScript 程序会尽可能修订业已存在的就命令行脚本文件。**
